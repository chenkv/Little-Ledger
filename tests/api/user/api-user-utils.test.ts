...
describe("user utils", () => {
  ...
  it("returns null for an expired session token", async () => {
    ...
    insertSession.run(userId, "expired-token", new Date(Date.now() - 1000).toISOString());
  ...
  it("returns the user id for a valid session token", async () => {
    ...
    insertSession.run(userId, "valid-token", new Date(Date.now() + 1000 * 60 * 60).toISOString());
